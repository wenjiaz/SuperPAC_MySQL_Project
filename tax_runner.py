...
def insertLine(aDict):
    #possible optimization: INSERT ON DUPLICATE KEY or LOAD LOCAL INFILE
    query="""REPLACE INTO super_pac_backup (form_id, org, EIN, nameLast, nameFirst, compName,add1, add2, city, state, zip, zipExt, employer, amount, occupation, aggTotal, contDate, present) VALUES (%s, %s, %s,%s, %s, %s,%s, %s, %s,%s, %s, %s,%s, %s, %s,%s, %s, %s) """ 
        ...
        for line in zippy.open('FullDataFile.txt'):
            if line[0]=='A':
                ...
                if len(foo)==x:
                    ...
                    # if value for the category is empty string, put in Null. 
                    for key in fullList:
                        value=lineDict[key]
                        if value==" " or value=="":
                            lineDict[key]=None
                    ...
                    addToLog("Skipped line:\n %s\n" % foo)
    except MySQLdb.Error, e:
        addToLog("Error in parseLine")
        raise
...
cursor.execute("""CREATE TABLE IF NOT EXISTS super_pac_backup ( form_id INT PRIMARY KEY, org VARCHAR(100), EIN INT, nameLast VARCHAR(40), nameFirst VARCHAR(40), compName VARCHAR(60), add1 VARCHAR(40), add2 VARCHAR(40), city VARCHAR(25), state VARCHAR(4), zip INT, zipExt INT, employer VARCHAR(90), amount INT, occupation VARCHAR(70), aggTotal INT, contDate INT, present TINYINT(1))""")
cursor.execute("""UPDATE super_pac_backup SET present=0 ;""")
...
if file:
    ...
    if result:
        ...
        cursor.execute("""DELETE FROM super_pac_backup WHERE present=0 ;""")
